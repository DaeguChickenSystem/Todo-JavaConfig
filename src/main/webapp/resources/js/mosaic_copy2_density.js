...
/*
		...
		for (var i = 1; i < (this.meta.width/density)*(this.meta.height/density); i++) {
			...
			if(density==5){
				$(div).css("background-size", this.meta.width*density*4 +"% "+this.meta.height*density*4+"%");
			}else if(density==10){
		...
		if (selected.length == 1) {
			...
			/*			if (opacity_num > 4) {
						...
						opacity_num++;*/
						 ...
						 $('.mosaic-area').css("width",  (Math.abs($(event.currentTarget).offset().left) - selected[0].x));
						 $('.mosaic-area').css("height",  (Math.abs($(event.currentTarget).offset().top)- selected[0].y));
	
					} else {
						// 사진원복
		/*				$(e).css("background", 'url(/resources/img/wb.jpg)');	
						if(density==5){
							$(e).css("background-size", $this.meta.width*density*4 +"% "+$this.meta.height*density*4+"%");
						}else if(density==10){
...
*/
	...
	function onclick(event) {
		...
		if (selected.length == 0) {	 
			...
			var y_distance = Math.abs(selected[1].y - selected[0].y) / 2;
