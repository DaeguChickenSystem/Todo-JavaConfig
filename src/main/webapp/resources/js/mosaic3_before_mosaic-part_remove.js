...
/*		event.preventDefault();
		...
		if ($this.selected.length == 1) {
			...
			$('.mosaic-area').css("width", (mouse_left - $this.selected[0].x));
			$('.mosaic-area').css("height", (mouse_top - $this.selected[0].y));
			
	
			
			if ($this.shape == "circle"){ 
				$('.mosaic-area').css("border-radius", (mouse_top - $this.selected[0].y)/2);			
			}else{
				$('.mosaic-area').css("border-radius", '');	
			}
...
//			event.preventDefault();
		...
		if ($this.selected.length == 0) {	 
			...
			console.log(mouse_top);
			 ...
			 $('.mosaic-area').css("left",Math.abs(Math.abs($(event.currentTarget).offset().left)));
			 $('.mosaic-area').css("top",Math.abs(Math.abs($(event.currentTarget).offset().top)));
			 $('.mosaic-area').css("background-position-x", -(mouse_left-243));
			 $('.mosaic-area').css("background-position-y", -(mouse_top-164));
			...
			[].forEach.call($('.mosaic-part'), function(e) {
				...
				if ($this.shape == "square") {
					if (x_value > ($this.selected[0].x - 1)&& x_value < (Number($this.selected[1].x) + 1)) {
						$this.selectedX.push({
							x : x_value,
							y : y_value
						});

						if (y_value > ($this.selected[0].y - 1)&& y_value < ($this.selected[1].y + 1)) {
							$this.selectedY.push({
								x : x_value,
								y : y_value
							});
				...
				/*			 $(e).css("background",
							 'url(/resources/img/wb.jpg)');
								...
								$(e).css("background-position-x", ($this.meta.width - ((x_value_%Math.abs(($this.meta.width/$this.density))) * $this.density)) + "px");
								$(e).css("background-position-y", ($this.meta.height - ((y_value - 1) * $this.density)) + "px");
							 $(e).css("background-size", $this.meta.width*$this.density*4 +"% "+$this.meta.height*$this.density*4+"%");
*/
							
						}
					}
				}else if($this.shape == "circle"){
					var radius = Math.abs($this.selected[1].x - $this.selected[0].x) / 2;
					var center = [Math.abs($this.selected[1].x +$this.selected[0].x) / 2, Math.abs($this.selected[1].y + $this.selected[0].y) / 2 ];
											
							/*isInCircle*/
							if(Math.pow(radius,2) >= Math.pow(center[0]-x_value,2) + Math.pow(center[1]-y_value,2)){
								if (opacity_num > 3) {
									if (($this.selected[1].x) % 2 == 0) {
										opacity_num = 1;
									} else {
